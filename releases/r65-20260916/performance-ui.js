(async()=>{
 ...
 if(integrated){
  ...
  window.hallChooseLighting=mode=>{
   ...
   if(mode!=='stage'||warmDone){chooseLighting(mode);return;}
   const status=document.querySelector('#lighting-dialog .light-status');
   status.hidden=false;status.classList.add('busy');status.textContent='正在準備演出燈光…';
   manualLightJob=warm(true).then(()=>{if(!session.active)chooseLighting(mode);}).catch(()=>{
